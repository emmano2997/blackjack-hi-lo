...
def load_ranks(filepath):
    ...
    for Rank in ['Ace','Two','Three','Four','Five','Six','Seven',
        ...
        img_path = filepath + Rank + '.jpg'
        img = cv2.imread(img_path, cv2.IMREAD_GRAYSCALE)
        if img is None:
            print(f"[ERRO] Imagem de rank '{img_path}' não encontrada.")
        else:
            rank = Train_ranks()
            rank.name = Rank
            rank.img = img
...
def load_suits(filepath):
    ...
    for Suit in ['Spades', 'Diamonds', 'Clubs', 'Hearts']:
        img_path = filepath + Suit + '.jpg'
        img = cv2.imread(img_path, cv2.IMREAD_GRAYSCALE)
        if img is None:
            print(f"[ERRO] Imagem de suit '{img_path}' não encontrada.")
        else:
            suit = Train_suits()  # Corrigido: era Train_ranks()
            suit.name = Suit
            suit.img = img
            train_suits.append(suit)  # Corrigido: era train_suit
    return train_suits  # Corrigido: era train_suit
    ...
    if (len(qCard.rank_img) != 0) and (len(qCard.suit_img) != 0):
        ...
        for Trank in train_ranks:
                if qCard.rank_img.shape != Trank.img.shape:
                    print(f"Dimensões diferentes: {qCard.rank_img.shape} vs {Trank.img.shape}")
                    continue

                if qCard.rank_img is None or Trank.img is None:
                    print(f"Erro: Imagem inválida para comparação. Rank: {Trank.name}")
                    continue
